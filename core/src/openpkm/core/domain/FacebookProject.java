...
public class FacebookProject implements Domain, FacebookPage, PropertiesProvider, SourceProviders, BatchUpdateSupport
{
    ...
    public FacebookProject(FileObject projectDir, ProjectState state, Properties props) 
    {
        ...
        WebPageFactory webPageProvider = Lookup.getDefault().lookup(WebPageFactory.class);
        if(webPageProvider != null)
        {          
            SourceProvider links = new WebPageProviderImpl(webPageProvider);
            sources.put(links.getName(), links);            
        }        
        
        ReferenceFactory referenceProvider = Lookup.getDefault().lookup(ReferenceFactory.class);
        if(referenceProvider != null)
        {          
            SourceProvider references = new ReferenceProviderImpl(referenceProvider);
